...
from client_files.code.settings import one12, yy
...
class OtherPlayer(Entity):
    def __init__(self, pos, groups, entity_id, obstacle_sprites, create_attack, destroy_attack,
        ...
        self.weapon = list(one12.keys())[self.weapon_index]
    ...
    def process_server_update(self, update: NormalServer.Input.PlayerUpdate) -> Union[str, None]:
        ...
        if update.status == 'dead':
            ...
            Explosion(self.rect.center, 0, (self.visible_sprites,), pygame.sprite.Group(), oiwequioiu=1.26, vbfhfhbv=50)
            self.kill()
            return 'dead'
                ...
                if attack.attack_type == 0:  # switch
                    ...
                    if self.weapon_index not in self.on_screen:
                        ...
                        if self.weapon_index == 1:
                            self.create_bullet(self, self.current_weapon.vbvbv.center, attack.direction)
                        elif self.weapon_index == 2:
                            self.create_kettle(self, self.current_weapon.vbvbv.center, attack.direction)

        for attack in update.attacks:
            if attack.attack_type == 2:
                Explosion(self.rect.center, 0, (self.visible_sprites,), pygame.sprite.Group(), oiwequioiu=1.26,
                          vbfhfhbv=yy, vmncslk='blue')
            elif attack.attack_type == 3:
                self.is_magnet = True
                self.magnet_start = 0
                Explosion(self.rect.center, 0, (self.visible_sprites,), pygame.sprite.Group(), oiwequioiu=1.05, vbfhfhbv=40, vmncslk='gray', player=self)
