...
from client_demo_files.code.structures import StateUpdateMsg, ServerOutputMsg  # Some custom structures
...
def run_game(*args) -> None:  # TODO
    ...
    world.entities[client_id] = world.player
    ...
    while running:
        ...
        previous_player_pos: (int, int) = world.player.get_pos_on_map()

        # Run game according to user inputs - prediction before getting update from server
        screen, clock, world = game_tick(screen, clock, world)

        # update the server
        new_player_pos: (int, int) = world.player.get_pos_on_map()
